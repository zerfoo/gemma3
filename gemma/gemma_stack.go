...
func NewGemmaStack[T tensor.Numeric](
	...
	for i := 0; i < numLayers; i++ {
		...
		if (i+1)%globalInterval == 0 {
			...
			attentionLayer, err = attention.NewLocalAttention[T](
				engine, ops, modelDim, numQueryHeads, numKeyValueHeads, localWindowSize,
				attention.WithLocalRopeBase[T](base),
				attention.WithLocalMaxSeqLen[T](maxSeqLen),
			)
		}
		if err != nil {
		...
		block, err := transformer.NewTransformerBlock[T](engine, ops, modelDim, ffnDim, attentionLayer, transformer.WithEpsilon[T](epsilon))
		if err != nil {
